...
const OtherCaseStudies: React.FC<OtherCaseStudiesProps> = ({ otherCaseStudies }) => {
  return (
    ...
    >
      <h2 className="md:text-[38px] sm:text-[28px] text-[24px] leading-[1.4em] dark:text-primary text-black -tracking-[0.5px] mb-8 capitalize ">Explore Related Projects
      </h2>
      <div className="grid md:grid-cols-2 lg:grid-cols-2 sm:grid-cols-2 gap-4 md:gap-8">
          ...
          <Link href={`/work/${caseStudy.slug.current}`} key={caseStudy.slug.current}>
            <div className="border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300">
              ...
              <div className="p-3 md:p-6">
                ...
                <p className=" dark:text-primary text-black">{truncateText(caseStudy.caseStudyCardData.description, 120)}</p>
              </div>
            </div>
          </Link>
        ))}
